...
This starts a MySQL 8.0 database and 3 APIs with:
- Container: `rent-mysql`
- Port: `3306`
- Databases: `rent_db` (properties), `rent_car` (cars)

- Container: `rent-front-api`
- Port: `8080`

- Container: `rent-properties-api`
- Port: `8081`

- Container: `rent-cars-api`
- Port: `8082`
...
### Front API - `http://localhost:8080/front-api`
```bash
curl -X GET http://localhost:8080/front-api/rental-properties
```

#### Get Property by ID
```bash
curl -X GET http://localhost:8080/front-api/rental-properties/1
```

#### Create Property
```bash
curl -X POST http://localhost:8080/front-api/rental-properties \
...
```bash
curl -X PUT http://localhost:8080/front-api/rental-properties/1 \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X PATCH http://localhost:8080/front-api/rental-properties/1 \
  -H "Content-Type: application/json" \
  -d '{
    "rentAmount": 1300.0,
    "town": "Marseille"
...
#### Delete Property
```bash
curl -X DELETE http://localhost:8080/front-api/rental-properties/1
```

#### Get All Cars
```bash
curl -X GET http://localhost:8080/front-api/rental-cars
```

#### Get Car by ID
```bash
curl -X GET http://localhost:8080/front-api/rental-cars/1
```

#### Create Car
```bash
curl -X POST http://localhost:8080/front-api/rental-cars \
  -H "Content-Type: application/json" \
  -d '{
    "brand": "Peugeot",
    "model": "308",
...
#### Update Car (Full)
```bash
curl -X PUT http://localhost:8080/front-api/rental-cars/1 \
  -H "Content-Type: application/json" \
  -d '{
    "brand": "Renault",
    "model": "Clio",
...
#### Update Car (Partial)
```bash
curl -X PATCH http://localhost:8080/front-api/rental-cars/1 \
  -H "Content-Type: application/json" \
  -d '{
    "rentAmount": 50.0
  }'
...
#### Delete Car
```bash
curl -X DELETE http://localhost:8080/front-api/rental-cars/1
```
